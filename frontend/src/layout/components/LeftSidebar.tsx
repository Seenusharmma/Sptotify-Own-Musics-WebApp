...
const LeftSidebar = () => {
	...
	}, [fetchAlbums, fetchFeaturedPlaylists]);
				...
				<ScrollArea className='h-[calc(100vh-300px)]'>
					<div className='space-y-2'>
							...
							[...albums, ...featuredPlaylists].map((album) => (
								...
								>
									...
									<div className='flex-1 min-w-0 hidden md:block'>
										...
										<p className='text-sm text-zinc-400 truncate'>{album._id.startsWith('jio-playlist-') ? 'Playlist' : 'Album'} • {album.artist}</p>
									</div>
								</Link>
							))
						)}
                        <Link
                            ...
                            className='p-2 hover:bg-zinc-800 rounded-md flex items-center gap-3 group cursor-pointer'
                        >
                             <div className="size-12 rounded-md flex items-center justify-center bg-gradient-to-br from-indigo-700 to-indigo-900 flex-shrink-0">
